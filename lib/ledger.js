...
export const DEFAULT_LEVELS = {
    ...
    调整日程: 'L1',
    发送消息: 'L1',
    // 内部开发动作（主人拍板：分身做开发=正常工作，L1 放行留痕——
    // 这类动作不涉对外承诺/不可逆操作，逐次审批会让开发任务永久卡死）
    开发: 'L1',
    修复: 'L1',
    重构: 'L1',
    编码: 'L1',
...
export const UNKNOWN_ACTION_LEVEL = 'L2';
const LEVEL_ORDER = { L0: 0, L1: 1, L2: 2, L3: 3 };
/** 裁决动作类型级别；策略卡显式 levelHint 优先，未知动作类型兜底 L2 且不允许降级到 L2 以下 */
export function decideLevel(actionType, levelHint) {
    const key = String(actionType ?? '').trim();
    const known = DEFAULT_LEVELS[key];
    const hint = typeof levelHint === 'string' && LEVELS.includes(levelHint) ? levelHint : undefined;
    if (known === undefined) {
...
export function saveLedger(store) {
    const path = ledgerPath();
...
/* ── 可注入时钟（测试 TTL） ── */
let nowFn = () => new Date();
export function setNowForTest(fn) {
    nowFn = fn;
}
function now() {
    return nowFn();
...
export function grantCovers(grant, actionType, targetScope) {
    if (grant.revokedAt !== undefined)
        return false;
    if (grant.expiresAt !== undefined && grant.expiresAt <= now().toISOString())
        return false;
    if (grant.actionType !== actionType)
        return false;
    if (grant.targetScope === '*')
        return true;
    return targetScope.includes(grant.targetScope);
}
/** 纯裁决：给定级别与当前有效授权，判定放行/阻断/拒绝（无 IO，契约测试锁定） */
export function judge(level, grants, actionType, targetScope) {
    if (level === 'L3') {
...
export function normalizeCheckInput(input) {
    ...
    if (actionType === '')
        throw new Error('actionType 不能为空');
    const targetScope = String(input.targetScope ?? '')
        .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, '')
        .trim()
        .slice(0, 500);
    if (targetScope === '')
        throw new Error('targetScope 不能为空');
    const actor = {};
    if (input.actor !== null && typeof input.actor === 'object') {
        const a = input.actor;
        if (typeof a.registryId === 'string' && a.registryId.trim() !== '')
            actor.registryId = a.registryId.trim().slice(0, 60);
        if (typeof a.channel === 'string' && a.channel.trim() !== '')
            actor.channel = a.channel.trim().toLowerCase().slice(0, 40);
    }
...
function appendHistory(record, status, via) {
...
function expireApprovalsUnlocked(store) {
...
/** 拒绝：记录终态「已拒绝」，不产生任何授权 */
export function rejectApproval(approvalId, by = {}) {
    const store = loadLedger();
    const expired = expireApprovalsUnlocked(store);
    const approval = store.approvals.find(p => p.id === approvalId);
    if (approval === undefined)
...
export function markExecuted(recordId) {
    const store = loadLedger();
    const record = store.records.find(r => r.id === recordId);
    if (record === undefined)
        return { ok: false, error: '记录不存在' };
    if (record.status !== '已放行')
        return { ok: false, error: `记录状态为「${record.status}」，不能标记执行` };
    appendHistory(record, '已执行', '工具执行完成');
    saveLedger(store);
    return { ok: true, record };
}
export function markExecutedForAction(actionType, targetScope) {
    const store = loadLedger();
    const candidates = store.records.filter((r) => r.actionType === actionType && r.status === '已放行' && (targetScope === undefined || r.target.scope === targetScope));
    const record = candidates[candidates.length - 1];
    if (record === undefined)
        return { ok: false, error: '无匹配的已放行记录' };
    appendHistory(record, '已执行', '工具执行完成');
    saveLedger(store);
    return { ok: true, record };
}
const FEEDBACKS = ['认可', '推翻', '未评价'];
/** 结果回填（决策二性质 4）：每次放行的动作执行后强制记录实际结果与主人反馈 */
export function fillResult(recordId, input) {
    const summary = String(input.summary ?? '').trim().slice(0, 500);
    if (summary === '')
        return { ok: false, error: '结果摘要不能为空' };
    const feedback = FEEDBACKS.includes(input.masterFeedback)
        ? input.masterFeedback
        : '未评价';
    const store = loadLedger();
    const record = store.records.find(r => r.id === recordId);
    if (record === undefined)
    ...
    if (record.status !== '已回填')
        appendHistory(record, '已回填', '结果回填');
    saveLedger(store);
    return { ok: true, record };
}
/** 主人反馈更新（认可/推翻）：推翻记录自动成为策略卡修订的候选证据 */
export function feedback(recordId, masterFeedback, note) {
    if (!FEEDBACKS.includes(masterFeedback)) {
    ...
    if (record.outcome === undefined)
        return { ok: false, error: '该记录尚未回填结果，请先 fillResult' };
    record.outcome.masterFeedback = masterFeedback;
    if (note !== undefined)
        record.outcome.summary = `${record.outcome.summary}（${String(note).slice(0, 200)}）`;
    saveLedger(store);
    return { ok: true, record };
}
export function revoke(grantId, reason) {
    const store = loadLedger();
...
export function records(filter = {}) {
    const store = loadLedger();
...
export function grants(onlyActive = true) {
    const store = loadLedger();
    const nowIso = now().toISOString();
...
export function pendingApprovals() {
    ...
    if (store.approvals.some(p => p.state === '已过期'))
        saveLedger(store);
...
export function stats() {
    const store = loadLedger();
    const byStatus = { 已放行: 0, 已阻断: 0, 已拒绝: 0, 已执行: 0, 已回填: 0 };
    let approved = 0;
    let overridden = 0;
    for (const r of store.records) {
        byStatus[r.status] = (byStatus[r.status] ?? 0) + 1;
        if (r.outcome !== undefined && r.outcome.masterFeedback === '认可')
            approved += 1;
        if (r.outcome !== undefined && r.outcome.masterFeedback === '推翻')
            overridden += 1;
    }
...
export function selfCheck() {
    ...
    if (judge('L2', [], '报价', 'A 客户').decision !== '阻断')
        issues.push('L2 无授权未阻断');
    if (judge('L2', [g], '报价', 'A 客户的询价').decision !== '放行')
        issues.push('L2 命中授权未放行');
    if (judge('L2', [g], '对外承诺', 'A 客户').decision !== '阻断')
        issues.push('授权串类型未隔离');
    if (judge('L3', [g], '转账', '任何').decision !== '拒绝')
        issues.push('L3 未拒绝');
    if (decideLevel('不认识的动作') !== 'L2')
        issues.push('未知动作类型未兜底 L2');
    if (decideLevel('转账') !== 'L3')
        issues.push('转账默认级别错误');
