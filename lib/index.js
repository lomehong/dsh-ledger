 ...
 */
import { approve, check, feedback, fillResult, grants, judge, loadLedger, markExecuted, normalizeCheckInput, pendingApprovals, records, rejectApproval, markExecutedForAction, revoke, selfCheck, setNowForTest, stats, } from "./ledger.js";
    ...
    try {
        if (typeof c.on === 'function') {
            const handler = (payload) => {
                ...
                const p = payload;
                const actionType = typeof p?.args?.actionType === 'string' ? p.args.actionType : typeof p?.tool === 'string' ? p.tool : '';
                if (actionType === '')
                    return { action: 'allow' };
                const args = (p?.args ?? {});
                const input = {
                ...
                if (result.judgment.decision === '放行')
                    return { action: 'allow', recordId: result.record.id };
                return {
                    action: 'deny',
